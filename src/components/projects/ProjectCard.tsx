      ...
      {project.caseStudies && project.caseStudies.length > 0 ? (
        <div className="mt-3 flex flex-wrap gap-3">
          {project.caseStudies.map((caseStudy) =>
            caseStudy.disabled || !caseStudy.href ? (
              <span
                key={caseStudy.label}
                aria-disabled="true"
                className="cursor-not-allowed rounded-lg border border-slate-200 bg-slate-100 px-4 py-2 text-sm font-medium text-slate-500"
              >
                {caseStudy.label}
              </span>
            ) : (
